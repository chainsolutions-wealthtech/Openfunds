...
- [x] **Step 1: Write failing tests** vérifiant les sept sources, 18 domaines, le décompte exact par domaine, 420 lignes et l’unicité de tous les codes.
- [x] **Step 2: Verify tests fail** avant création du parseur.
- [x] **Step 3: Implement strict Markdown parsing** sans traduction ni normalisation destructive ; conserver texte, fréquence, unité, source préférée, usages, benchmark, priorité, historique et colonnes D00 supplémentaires.
- [x] **Step 4: Encode absence explicitly** : valeurs non présentes = `null`, accompagnées de statuts `NOT_AUTHORED` lorsque requis.
- [x] **Step 5: Verify tests pass** et que toute variation de compte/colonne/source échoue.
...
- [x] **Step 1: Bootstrap exactly from legacy sources** ; aucune définition ou traduction nouvelle.
- [x] **Step 2: Store provenance per object** : fichier source, domaine, code et hash/source locator déterministe.
- [x] **Step 3: Separate source and governed semantics** avec `source_nature` et `canonical_nature` ; lorsqu’une classification gouvernée n’est pas explicitement décidée, utiliser `null` + `NOT_AUTHORED`.
- [x] **Step 4: Store independent evidence states** : `definition_status`, `source_mapping_status`, `collection_spec_status`, `collection_test_status`, `history_status`, `calculation_status`.
- [x] **Step 5: Validate JSON package** contre le schéma logique et le snapshot legacy.
...
- [x] **Step 1: Write tests for deterministic ordering and CSV delimiter `;`.**
- [x] **Step 2: Generate exhaustive Markdown** par D00–D17 depuis le JSON master.
- [x] **Step 3: Generate SQL seed** créant/peuplant uniquement les objets catalogue requis de manière additive/idempotente, sans observations ni données pays.
- [x] **Step 4: Generate manifest checksums** et faire échouer `--check-manifest` sur toute dérive.
- [x] **Step 5: Verify two consecutive generations are byte-identical.**
...
- [x] **Step 1: Test migration shape first** : aucune commande destructive, aucune observation, aucune donnée pays, aucune transaction embarquée.
- [x] **Step 2: Define `ref.indicator_domain` and `ref.indicator_definition`** avec code stable, sémantique source, statuts et contraintes d’unicité.
- [x] **Step 3: Seed exactly 18 domains and 420 definitions** depuis le générateur ; `ON CONFLICT` ne doit jamais masquer une collision sémantique.
- [x] **Step 4: Add migration to governed manifest at order 160** avec mode généré/frozen selon la politique existante.
- [x] **Step 5: Verify migration materialization and runner plan** sans connexion production.
...
- [x] **Step 1: CI runs generation + manifest check + unit tests on Python 3.11 and 3.12.**
- [x] **Step 2: CI uploads JSON/CSV/Markdown/SQL build views** for review.
- [x] **Step 3: Document source-of-truth transition** : Markdown historiques = bootstrap/audit ; JSON v1 = authoring authority ; build = derived.
- [x] **Step 4: Mark `OF-DATA-003` complete only after CI success** et ne démarrer aucune tâche suivante automatiquement dans la même clôture.
- [x] **Step 5: Record dynamic HEAD policy and exact restart gate** pour la prochaine boucle.
