...
- [ ] **Step 1:** Résoudre le HEAD courant et vérifier qu’il descend sans divergence de la baseline documentaire.
- [ ] **Step 2:** Compter les pays ayant au moins une organisation country-scoped, les rôles couverts, les statuts `VALIDATED/PENDING`, et les pays sans organisation propre.
- [ ] **Step 3:** Réconcilier les candidats de la PR #2 avec les codes existants et classifier les collisions.
- [ ] **Step 4:** Vérifier chaque organisation de l’allowlist sur une source officielle primaire actuelle.
- [ ] **Step 5:** Écrire le rapport daté sans modifier les registres canoniques.
...
- [ ] **Step 1: RED:** ajouter les tests d’unicité, uppercase/no-accent, FK organisation→rôle/scope, scope country/zone et présence d’URL pour les organisations/endpoints `VALIDATED`.
- [ ] **Step 2:** Exécuter la CI et confirmer que le test de couverture de l’allowlist échoue tant que les nouvelles organisations ne sont pas intégrées.
- [ ] **Step 3: GREEN:** implémenter le validateur minimal et son workflow Python 3.11/3.12.
- [ ] **Step 4:** Confirmer que les invariants existants passent et que seule l’absence attendue de la vague 01 reste rouge.
...
- [ ] **Step 1:** Ajouter seulement les codes absents et prouvés : Botswana, Namibie, Éthiopie, AMF-UMOA, UMOA-Titres, COSUMAF selon l’allowlist finale.
- [ ] **Step 2:** Conserver `VALIDATED` uniquement quand identité + rôle principal + domaine officiel sont prouvés ; sinon `PENDING`.
- [ ] **Step 3:** Vérifier absence de doublon/code collision et exécuter le validateur.
...
- [ ] **Step 1:** Ajouter les relations prouvées `CENTRAL_BANK`, `STOCK_EXCHANGE`, `CAPITAL_MARKET_REGULATOR`, `FUND_REGULATOR`, `INSURANCE_PENSION_REGULATOR`, `STATISTICS_OFFICE`, `GOVERNMENT_SECURITIES_AGENCY` selon les sources officielles.
- [ ] **Step 2:** Ne pas ajouter `INDEX_PROVIDER`, `FX_REFERENCE_RATE_PROVIDER`, ministère/dette/assurance ou autre rôle non prouvé par la source contrôlée.
- [ ] **Step 3:** Exécuter le validateur et vérifier toutes les FK/scopes.
...
- [ ] **Step 1:** Revalider le HEAD et les workflows déclenchés.
- [ ] **Step 2:** Documenter faits, preuves, limites, risques et couverture restante.
- [ ] **Step 3:** Laisser `OF-SOURCE-002` en `EN_COURS` sauf si les 54 pays et tous les rôles exigés sont réellement couverts.
- [ ] **Step 4:** Fixer la prochaine vague en lecture seule avant toute nouvelle écriture.
